...
URL = "https://floridagators.com/sports/womens-basketball/roster"
...
for element in soup.find_all('span', {'class': 'sidearm-roster-player-first-name'}):
    first_names.append(element.text.strip())

for element in soup.find_all('span', {'class': 'sidearm-roster-player-last-name'}):
    last_names.append(element.text.strip())
...
db.child("rosters").child('womens_basketball').set(dictionaryList)